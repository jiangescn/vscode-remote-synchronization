...
Map<string, int> apportion(const Map<string, int> &populations, int numSeats)
{
    ...
    for (string state : populations)
    {
        ...
        double weight = populations[state] / sqrt(2.0);
    ...
    while (numSeats)
    {
        DataPoint top = pq.dequeue();
        string state = top.name;

        result[state]++;

        double newWeight = populations[state] / sqrt(double(result[state] * (result[state] + 1)));

        pq.enqueue({state, -newWeight});
