from __future__ import annotations # 这会使类型注解正常工作


SOURCE_FILE = __file__


def flatten(s: list) -> list:
    """返回列表 s 展平后的版本。

    >>> flatten([1, 2, 3])
    [1, 2, 3]
    >>> deep = [1, [[2], 3], 4, [5, 6]]
    >>> flatten(deep)
    [1, 2, 3, 4, 5, 6]
    >>> deep                                # 输入列表保持不变
    [1, [[2], 3], 4, [5, 6]]
    >>> very_deep = [['m', ['i', ['n', ['m', 'e', ['w', 't', ['a'], 't', 'i', 'o'], 'n']], 's']]]
    >>> flatten(very_deep)
    ['m', 'i', 'n', 'm', 'e', 'w', 't', 'a', 't', 'i', 'o', 'n', 's']
    """
    "*** 在此处编写代码 ***"
    ans: list = []

    for _ in s:
        if(type(_) == list):
            ans += flatten(_)
        else:
            ans += [_]

    return ans
            
            

def close_list(s: list[int], k: int) -> list[int]:
    """返回一个列表，其中包含 s 中与自身索引之差不超过 k 的元素。

    >>> t = [6, 2, 4, 3, 5]
    >>> close_list(t, 0)  # 只有 3 等于它的索引
    [3]
    >>> close_list(t, 1)  # 2、3 和 5 与各自索引之差不超过 1
    [2, 3, 5]
    >>> close_list(t, 2)  # 2、3、4 和 5 与各自索引之差都不超过 2
    [2, 4, 3, 5]
    """
    assert k >= 0
    return [s[i] for i in range(len(s)) if abs(s[i] - i) <= k]


def remove_first(lst: list, elem: int) -> list:
    """此函数删除列表 lst 中第一次出现的 elem。

    >>> remove_first([3, 4] , 3)
    [4]
    >>> remove_first([3, 4, 3] , 3)
    [4, 3]
    >>> remove_first([2, 4] , 3)
    [2, 4]
    >>> remove_first([] , 0)
    []
    """
    "*** 在此处编写代码 ***"
    ans:list = []
    first = True
    for i in range(len(lst)):
        if(lst[i] == elem and first):
            first = False
        else:
            ans += [lst[i]]
    
    return ans

def sort(lst: list) -> list:
    """此函数返回列表 lst 排序后的版本。

    >>> sort([6, 2, 5])
    [2, 5, 6]
    >>> sort([2, 3])
    [2, 3]
    >>> sort([3])
    [3]
    >>> sort([])
    []
    """
    "*** 在此处编写代码 ***"
    ans: list = []
    for _ in range(len(lst)):
        mn = min(lst)
        lst = remove_first(lst, mn)
        ans += [mn]
    return ans

def make_onion(f, g):
    """返回函数 can_reach(x, y, limit)，用于判断是否存在一个
    只包含 f、g 和 x、且最多调用 limit 次函数的调用表达式，
    能够得到结果 y。

    >>> up = lambda x: x + 1
    >>> double = lambda y: y * 2
    >>> can_reach = make_onion(up, double)
    >>> can_reach(5, 25, 4)      # 25 = up(double(double(up(5))))
    True
    >>> can_reach(5, 25, 3)      # 无法实现
    False
    >>> can_reach(1, 1, 0)      # 1 = 1
    True
    >>> add_ing = lambda x: x + "ing"
    >>> add_end = lambda y: y + "end"
    >>> can_reach_string = make_onion(add_ing, add_end)
    >>> can_reach_string("cry", "crying", 1)      # "crying" = add_ing("cry")
    True
    >>> can_reach_string("un", "unending", 3)     # "unending" = add_ing(add_end("un"))
    True
    >>> can_reach_string("peach", "folding", 4)   # 无法实现
    False
    """
    def can_reach(x, y, limit: int) -> bool:
        if limit < 0:
            return False
        elif x == y:
            return True
        else:
            return can_reach(g(x), y, limit - 1) or can_reach(f(x), y, limit - 1)
    return can_reach


def make_func_repeater(f, x: int):
    """
    >>> increment_repeater = make_func_repeater(lambda x: x + 1, 1)
    >>> increment_repeater(2) # 等同于 f(f(x))
    3
    >>> increment_repeater(5)
    6
    """
    def repeat(n):
        if n == 0:
            return x
        else:
            return f(repeat(n - 1))
    return repeat


def ten_pairs(n: int) -> int:
    """返回正整数 n 中和为 10 的数字对的数量。

    >>> ten_pairs(7823952) # 7+3、8+2 和 8+2
    3
    >>> ten_pairs(55055)
    6
    >>> ten_pairs(9641469) # 9+1, 6+4, 6+4, 4+6, 1+9, 4+6 
    6
    >>> # 禁止使用迭代
    >>> from construct_check import check
    >>> check(SOURCE_FILE, 'ten_pairs', ['While', 'For'])
    True
    """
    "*** 在此处编写代码 ***"
    ans = 0

    ans += count_digit(n, 1) * count_digit(n, 9)
    ans += count_digit(n, 2) * count_digit(n, 8)
    ans += count_digit(n, 3) * count_digit(n, 7)
    ans += count_digit(n, 4) * count_digit(n, 6)

    count_5 = count_digit(n, 5)
    ans += count_5 * (count_5 - 1) // 2

    return ans



def count_digit(n: int, digit: int) -> int:
    """返回 digit 在 n 中出现的次数。

    >>> count_digit(55055, 5) # 数字 5 在 55055 中出现 4 次
    4
    >>> from construct_check import check
    >>> # 禁止使用迭代
    >>> check(SOURCE_FILE, 'count_digits', ['While', 'For'])
    True
    """
    "*** 在此处编写代码 ***"
    if n == 0:
        return 0

    if n % 10 == digit:
        return 1 + count_digit(n // 10, digit)
    else:
        return count_digit(n // 10, digit)
